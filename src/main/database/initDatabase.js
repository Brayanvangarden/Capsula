...
function hashLegacyPasswords(db) {
  ...
  console.log(
    `✅ Re-hashed ${needUpdate.length} contraseñas heredadas con bcrypt`,
  );
}

function repairLegacyClienteColumns(db) {
  const clientesTable = db
    .prepare(
  ...
  if (!clientesTable && legacyTable) {
    console.log(
      "⚠️ Se encontró clientes_legacy sin tabla clientes. Recuperando estructura principal...",
    );
    db.pragma("foreign_keys = OFF");

    try {
      db.exec("ALTER TABLE clientes_legacy RENAME TO clientes");
      console.log("✅ clientes_legacy restaurado como clientes");
      return;
    } finally {
  ...
  console.log(
    "⚠️ Se detectaron columnas legacy en clientes: apellido/cedula. Reparando estructura...",
  );
  db.pragma("foreign_keys = OFF");
