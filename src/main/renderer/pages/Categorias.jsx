...
function Categorias() {
  ...
  const totalPaginas = Math.max(
    1,
    Math.ceil(lista.length / CATEGORIAS_POR_PAGINA),
  );
  const paginaActual = Math.min(pagina, totalPaginas);
  const categoriasVisibles = lista.slice(
    (paginaActual - 1) * CATEGORIAS_POR_PAGINA,
    paginaActual * CATEGORIAS_POR_PAGINA,
  );
    ...
    <div className="page-container">
              ...
              <tbody>
                {categoriasVisibles.map((c, index) => (
                  <tr key={c.id}>
                    ...
                    <td>
                      <span
                        className={`pill ${c.estado === "activo" ? "pill-si" : "pill-no"}`}
                      >
      ...
      {confirmCategoria && (
        <div
          ...
          onClick={() => setConfirmCategoria(null)}
        >
