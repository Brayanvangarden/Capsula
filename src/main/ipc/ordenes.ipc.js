...
function registerOrdenesIpc() {
  ...
  // ── Editar orden con detalle ───────────────────────
  ipcMain.handle("ordenes:update", async (_, { id, data: formData }) => {
    try {
      const data = ordenesRepo.update(id, formData);
      return { ok: true, data, message: "Orden actualizada correctamente" };
    } catch (error) {
      return { ok: false, message: error.message };
    }
  });
